...
# PC_components = 21
# datapath="data/"
# glom_types = pd.read_csv('glomtypes.csv')
...
# # Return Caron weight matrix as a dataframe
# def get_Caronlike():
# 	Caron = pd.read_csv('CaronLabData_DMelanFemale_Translated.csv')
...
# Import any of the Caron datasets
...
# def get_FAFB():
# 	bin_mat = np.load('200514_pn_kc_bi_conn.npy') # Shape (1356, 113)
...
# 	types = pd.read_csv('180613-pn_subtypes.csv')
...
# def run_PCA(W_in, W_label, shuffle_type, title, against):
...
# 		# To sort by indices of first dataset
# 		#if i == 0:
# 	#		acp = acp.sort_values(ascending=False)
# 	#		inds = acp.index.values
# 	#	else:
# 	#		acp = acp.reindex(inds)
...
# 	# Exactly two weights passed in 
# 	if (len(list_weights) == 2):
# 		my_colors = glom_types['color']

# 		plt.xticks(rotation=90, fontsize=15)
# 		for ticklabel, tickcolor in zip(plt.gca().get_xticklabels(), my_colors):
# 			ticklabel.set_color(tickcolor)

# 		# To highlight significant gloms
# 		#pvals = compute_2propPvalues(list_weights)
# 		#for i in range(len(pvals['Glom'])):
# 		#	if (pvals['Significant Diff?'][i] == True):
# 		#		plt.axvspan(pvals['Glom'][i], pvals['Glom'][i+1], color='yellow', alpha = 0.5)
...
# def kc_subtypes():
# 	W_Neuprint, W_Neuprint_thr, W_Caron, W_Caron2013, W_Mel_Male, W_Mel_Female, W_Sec_Female, W_Sim_Female, W_FAFB_thr, cin_val, c, Caron = alignConnectomes()
